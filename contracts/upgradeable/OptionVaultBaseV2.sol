...
//In real case,  since we can change OptionVaultStorage to inherit from OptionVaultStorageV2
abstract contract OptionVaultBaseV2 is
    OptionVaultStorage, 
    ...
    ISettlementAggregator
{
    ...
    function isValidAddress(address _target, address _asset, uint256 _amount) private view {
        require(whitelist[_target][_asset] > _amount, "invalid target & amount"); 
    }

    function addWhitelistAddressInternal(address _target, address[] memory _assets, uint256[] memory _allowedWithdrawAmounts) internal {
        uint256 count = _assets.length;
        require(count == _allowedWithdrawAmounts.length, "inconsistent asset and amount");
        for(uint256 i = 0; i < count; i++) {
             whitelist[_target][_assets[i]] = _allowedWithdrawAmounts[i];
        }  
    }
    ...
    function withdrawAsset(address _trader, address _asset) external override lock settlerOnly { 
        ...
        isValidAddress(_trader, _asset, balance);
    ...
    function batchWithdrawAssets(address _trader, address[] memory _assets) external override lock settlerOnly{ 
        ...
        for(uint256 i = 0; i < count; i++) {
            ...
            require(assetSubData.leftOverAmount > 0); 
            uint128 balance = uint128(assetSubData.leftOverAmount);
            isValidAddress(_trader, _assets[i],  balance);
