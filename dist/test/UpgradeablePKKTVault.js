...
const USDTMultiplier = ethers_1.BigNumber.from(10).pow(USDTDecimals);
const USDCMultiplier = ethers_1.BigNumber.from(10).pow(USDCDecimals);
const DAIMultiplier = ethers_1.BigNumber.from(10).pow(DAIDecimals);
describe.skip("PKKT Vault (Proxy)", async function () {
    ...
    context("With vault added to the field", function () {
        beforeEach(async function () {
            ...
            this.owner = deployer;
            usdt = await (0, deploy_1.deployContract)("ERC20Mock", deployer, ["USDTToken", "USDT", ethers_1.BigNumber.from(10000).mul(USDTMultiplier), USDTDecimals]);
            usdc = await (0, deploy_1.deployContract)("ERC20Mock", deployer, ["USDCToken", "USDC", ethers_1.BigNumber.from(10000).mul(USDCMultiplier), USDCDecimals]);
            dai = await (0, deploy_1.deployContract)("ERC20Mock", deployer, ["DAIToken", "DAI", ethers_1.BigNumber.from(10000).mul(DAIMultiplier), DAIDecimals]);
            ...
            pkktVault = await (0, deployUpgradable_1.deployUpgradeableContract)("PKKTVault", { signer: deployer, libraries: { Vault: vault.address } }, [pkktToken.address, "100", 13603000, trader.address]);
        });
        ...
        it("should allow deposit and settle and withdraw", async function () {
            ...
            await (0, timer_1.advanceBlockTo)(1363109);
        ...
        it("should allow harvest pkkt reward", async function () {
            ...
            await pkktVault.connect(alice).deposit(0, ethers_1.BigNumber.from(10).mul(USDTMultiplier));
            await (0, timer_1.advanceBlockTo)(13603399);
            //settlement at 13601300, the reward will be calculated 
            await pkktVault.connect(trader).initiateSettlement("100", trader.address);
